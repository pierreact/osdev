// Userland ARP implementation (mirrors src/net/arp.c with userland includes)
#include "net/arp.h"
#include "net/l2.h"
#include "string.h"
...
int arp_process(void *l2ctx, ArpTable *t, uint8 *frame, uint32 frame_len) {
    ...
    if (ntohs(arp->hw_type) != ARP_HW_ETHER) return -1;
    if (ntohs(arp->proto_type) != ETH_TYPE_IPV4) return -1;
    if (arp->hw_len != ETH_ADDR_LEN || arp->proto_len != 4) return -1;
...
int arp_request(void *l2ctx, uint32 target_ip) {
    ...
    return ctx->backend.send(ctx->backend_ctx, pkt, sizeof(pkt));
}
